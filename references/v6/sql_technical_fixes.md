...
## Fix S20 — Quarta auditoria adversarial (2026-07-21): rótulo do lag, citação Curupira e resíduos finos

**Status:** APLICADO — 2026-07-21 (relatório completo em `AUDIT_4_2026-07-21.md`).

**Método:** replicação independente do pipeline inteiro a partir dos CSVs brutos (DuckDB 1.5.4, ambiente isolado), 54/54 esperados reproduzidos; todos os números de caso da prosa recomputados; robustez re-testada (top 10/20/50 idênticos nos 3 limiares; Spearman 0,985 com empates médios — válido também para o cenário sem limiar, que o registro anterior só havia verificado por overlap); citações externas re-verificadas por fetch/busca.

**Achados e correções (todos aplicados nesta passada):**
1. **Rótulo do lag errado em 4 lugares** (não pego por S13 nem S18/D2): "59,2% match same-year" descrevia a categoria `both` do R (pressão material em t **e** t−1); o match same-year total é **63,9%** (59,2 + 4,7). Corrigido em `02_marts.sql`, `03_analytics.sql`, `final_reference.md` §4 e `sql_explained.md` §3.3. O erro subestimava a validação do same-year join.
2. **Citação Curupira insustentável**: os números citados (196 autos/R$87,9M/30.592 ha) não existem em fonte localizável e o balanço oficial de 2 anos (Agência Pará, 09-10/02/2025) registra 471 autos e **521 mil ha embargados** — ordem de grandeza acima. Substituído pelo balanço oficial em `final_reference.md` §6, narrative, writing sample e neste registro.
3. **Citações antes sem fonte, agora resolvidas** (em vez de removidas): orçamento IBAMA (41%/86–92%/~500 fiscais → Observatório do Clima; corte ~43% → LAUT/InfoAmazonia); IBAMA-Apuí 2025 (release oficial gov.br/ibama: R$173M, 87 autos, ~27 mil ha); prisão do ex-vice-prefeito (Operação Máscara Rural, 12/03/2025; ClimaInfo). Tamoiotatá confirmada em espelhos estáticos (R$144.719.048; 16.176,09 ha; 164 termos). §11.4 atualizado: só (e) 1.717 e (f) fixture seguem abertos.
4. **Resíduo do patch P9 (AUDIT_3)**: mediana de GLR corrigida no narrative (4,65) mas não no `final_reference.md` §8 (dizia 4,6) — corrigido.
5. **Resíduos de protótipo**: denominador measured_gap mediana/p25 2,06/1,55 → 2,07/1,56 (registro empírico); Itaituba R$842,5M → "R$843M" no narrative (fronteira de arredondamento, mesmo critério do 4,65).
6. **Nota de reprodutibilidade ampliada** (`sql_explained.md` §4): o não-determinismo FP não afeta só `total_desmatado_km2` — `fine_values`/`egs` divergem no último bit (~1e-16) entre réplicas; comparações exigem tolerância.
7. **Higiene de git**: working tree sujo apenas por line endings (CRLF/LF; conteúdo idêntico, md5 igual ignorando CR) em 4 arquivos, incluindo o CSV do IPCA "versionado" — `.gitattributes` criado (`* -text`); commit a cargo do autor.
8. **Modos de falha silenciosa sem check (sugestões, não aplicadas)**: `null_fine_ibama_clean = 356`; unicidade de (geocode, year) em `prodes_clean`; nenhum check cobre `annual_summary` (18 × 805).
