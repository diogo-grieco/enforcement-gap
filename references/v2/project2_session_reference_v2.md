...
## Session Reference v2
**Last updated:** 2026-05-04  
**Status:** Pipeline v3 completo (Fix 8 + Fix 9 aplicados). Script R: bloco de carregamento e filtros IBAMA revisados e documentados. Pendente: bloco de datas + PRODES + análise de lag no script R; revisão analytics layer (streak + priority_score); staging fixes (municipios_ref + ipca_deflator); Power BI; narrativas finais; Barra do Bugres.
...
| Field | Type | Notes |
|---|---|---|
| year | DOUBLE → INTEGER | 2008–2025 |
| area km² | DOUBLE | Annual deforestation per municipality |
| mun | VARCHAR | Municipality name |
| geocode_ibge | DOUBLE → BIGINT → VARCHAR | 7-digit IBGE code |
| uf | LOGICAL | NA in all rows — not used |

**Coverage stats:**
- 14,490 rows | 800 unique municipalities | 18 years
- 9,740 observations com area > 0
- Distribuição (area > 0): min=0.01, p25=0.54, median=2.3, p75=10.3, max=797 km²
...
| DT_FATO_INFRACIONAL | Data do fato infracional — **69% NULL no dataset filtrado. NÃO USADA.** |
...
- PRODES: 800 municípios únicos | 18 anos
- IBAMA (após filtros, 2008–2025): 2,655 geocodes únicos
- Municípios-ano IBAMA sem match no PRODES: **5,689** — todos com geocode inexistente no PRODES. Confirmado: estão fora da Amazônia Legal. Nenhum caso de município da Amazônia Legal sendo perdido.
...
Calculada sobre ~30% dos registros com `DT_FATO_INFRACIONAL` não-nulo (n=17.243). Mediana 6 dias, média 235 dias — distribuição bimodal. 52.5% dos registros lavrados na mesma semana do fato; 20% com lag > 1 ano. Análise comprometida por seleção não-aleatória da base amostral e por medir pergunta errada (lag interno IBAMA, não alinhamento com PRODES).

**Distribuição por faixa de lag (base comprometida, informativa):**

| Faixa | n | % | Mediana multa |
|---|---|---|---|
| ≤7 dias | 9.047 | 52.5% | R$95k |
| 8–30 dias | 1.344 | 7.8% | R$152k |
| 31–365 dias | 3.412 | 19.8% | R$290k |
| 1–2 anos | 1.248 | 7.2% | R$209k |
| >2 anos | 2.183 | 12.7% | R$160k |
...
**Análise 3 — distribuição mensal de lavraturas:**
Uniforme ao longo do ano, pico setembro-outubro (11.1% e 10.3%). Ausência de concentração em janeiro-março descarta hipótese de resposta sistemática ao ano anterior. Pico na estação seca é consistente com enforcement e desmatamento ocorrendo no mesmo período do ano.

**Análise 4 — sensibilidade gap_absoluto:**
Com join t+1, 1.033 de 5.948 municípios-ano gap_absoluto (17.4%) mudariam de categoria. Não altera a decisão — confirma que streaks longos (≥3 anos) são robustos à incerteza de 1 ano.
...
SELECT
    ...
    -- Substitui DT_FATO_INFRACIONAL (69% NULL). Ver Fix 8.
...
> v1 (DT_FATO_INFRACIONAL, 69% NULLs): gap_absoluto 57.7% (8,366) — inflado artificialmente
...
> **Perfil novo:** os líderes atuais são municípios menores e sistematicamente menos visíveis — Maranhão, municípios do interior do AM e PA — com desflorestamento acumulado moderado mas streaks muito longas. Nenhum deles aparece em operações IBAMA federais de grande escala. Isso representa o outro lado da impunidade: não os casos famosos, mas os casos invisíveis.
...
5. **Viés de volume no pipeline antigo.** O pipeline original (usando `DT_FATO_INFRACIONAL`, 69% NULL) capturava preferencialmente enforcement em municípios com alto volume de autos — os casos famosos mantinham registros suficientes mesmo a 28–40% de completude. O pipeline atual é mais completo e menos enviesado.
...
1. Revisão script R de exploração (entender e fechar cada decisão antes do commit)
2. Staging fixes: `municipios_ref` + `ipca_deflator` → commit 01_staging.sql
3. Marts: review + checks → commit 02_marts.sql
4. Analytics: revisão streak + priority_score (incluindo streak=1 bug) + p75 + checks → commit 03_analytics.sql
5. Documentação: README (pipeline final apenas), apêndice metodológico, narrativas finais, Barra do Bugres
6. Power BI: conexão + dashboard
7. Website + LinkedIn (após P1–P3 completos)
