...
PUB_N_ZERO_DEFOR <- 133     
PUB_N_AT_FLOOR   <- 106     
PUB_SPEARMAN_AGG <- 0.503   
...
p_scatter <- ggplot(formula_df, aes(x = numerador_18y, y = denominador_18y)) +
  ...
  annotate("text", x = X_MAX, y = FORMULA_FLOOR,
           label = paste(number(PUB_N_AT_FLOOR), "no piso da fórmula"),
           hjust = 1, vjust = -0.6, colour = "grey45", size = 2.8) +
  ...
  labs(x = "Numerador: km² desmatados",
       y = "Denominador: média geométrica de log(1+autos) e log(1+multas)") +
  theme_chart +
...
p_anchors <- ggplot(anchor_series, aes(x = year, y = egs)) +
  ...
  geom_point(aes(fill = egs == 0), shape = 21, colour = EGS_RAMP[5],
             stroke = 0.6, size = 1.6, show.legend = FALSE) +
  scale_fill_manual(values = c(`FALSE` = unname(EGS_RAMP[5]),
                               `TRUE`  = "white")) +
