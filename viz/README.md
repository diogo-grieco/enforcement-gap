...
loads the feeds with `stopifnot()` guards — mirroring the style of
...
builds it if it doesn't — so running `04` before `07` just saves `07` from
...
|---|---|---|
| `00_build_mesh.R` | — | municipal mesh GeoJSON in `data/data_ibge/` (one-time) |
