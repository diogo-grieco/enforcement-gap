...
Gera as 11 figuras dos relatórios em `deliverables/`, a partir dos parquets do pipeline. Cada script começa com `source("viz/00_setup.R")`, que declara caminhos, constantes de integridade, paleta e tema, e carrega os dados com `stopifnot()`, no mesmo estilo de `exploration/exploring_script.R`.
...
|---|---|---|
| `00_setup.R` | todos os scripts numerados | sempre; leitura barata dos parquets e da malha, bandas de direção do EGS, paleta, tema |
...
A suíte lê apenas os três parquets do pipeline e a malha. Nenhum script abre os CSVs brutos do IBAMA, e não há cache intermediário a construir.
...
- A direção do EGS (`egs_trend`) é classificada uma vez em `00_setup.R` e lida pelas figuras 5 e 8, que desenham a mesma grandeza em suportes diferentes. Antes cada uma classificava por conta própria, com vocabulários que discordavam. O cinza é `avg_egs_3y == 0`, não `n_years_pressure == 0`: onde a pressão cessou não há direção a medir.
