...
- Os modelos de painel do `04` reproduzem o piloto em Python usando `fixest`, com efeitos fixos de município e ano e erros-padrão clusterizados por município: o estimador previsto para a tese.
...
- **Duas exceções declaradas** à regra de que a suíte lê e desenha, e elas não são do mesmo tipo. A primeira é o Spearman de 0,503 em `02_ranking_panels.R`. Ele não descreve o índice, descreve a própria figura 1, medindo o quanto a razão dos totais de dezoito anos diverge do índice publicado, que é a média das razões anuais. É um número que só existe porque a figura existe, e por isso mora com ela.
- A segunda é o `04_panel_effects.R` inteiro, que estima sete regressões de painel antes de desenhar as figuras 10 e 11. Diferente do 0,503, este **poderia** morar a montante; só não pode morar no pipeline, porque DuckDB não tem estimador de efeitos fixos, e escrever à mão em SQL a transformação interna, a matriz sanduíche agrupada e os graus de liberdade seria mais lento, ilegível e não auditável, além de quebrar a correspondência com o estimador que a tese vai usar. A alternativa seria uma camada nova só para ele, e a decisão foi que um arquivo declarado custa menos que uma pasta com um morador. O que substitui a pureza aqui é o contrato: os vinte coeficientes que os relatórios publicam estão fixados como constantes `PUB_*` ao lado do modelo e asseridos logo abaixo, e o próprio arquivo é dividido em duas metades, estimação e figuras, com o separador explícito.
